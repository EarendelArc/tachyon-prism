...
generation 配置以已验证的只读描述符持续持有。Unix 上的 Xray 校验、候选启动和回滚使用固定
的子进程描述符，并通过 `-config /dev/fd/198` 读取；generation 自有描述符均使用 close-on-exec
语义，但这不声称会全局关闭进程中的其他描述符。展示路径仅用于诊断，不再是配置交付授权依据。
启动前会持续持有源文件与 root binding，启动后重新核验身份；后置核验失败会立即
